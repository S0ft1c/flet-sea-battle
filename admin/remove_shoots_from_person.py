...
def remove_shoots_from_person(page: ft.Page):
    def field_selected(event):
        ...
        if not f_info:
            # if there's no field -> no need in changing
            page.go('/admin_panel')
            db.close_connection()
        else:
            f_info: dict = json.loads(f_info)
            f_info[str(field_id)] = -1 + f_info.get(str(field_id), 0)
            if f_info[str(field_id)] < 0:  # if all of this will change below zero -> go out
                page.go('/admin_panel')
                db.close_connection()
    ...
    if page.session.contains_key('login_admin'):
        ...
        for idx, field in enumerate(fields):
            page.views[-1].controls[1].controls.append(
                ft.Column(controls=[
                    ft.ElevatedButton(
                        text=f'Удалить выстрел из поля номер {field[0]}',
                        color='black200',
